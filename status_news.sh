...
# crontab -e
# *    * * * *     sh /home/dionysus/.suckless/arch-dwmstatus/status_news.sh >> /dev/null

# dwmstatus read from ~/.notification.status
FILEPATH=~/.notification.status
:> $FILEPATH

# :> /tmp/.notification.status.new
# :> /tmp/.notification.status.new
# :> /tmp/.notification.status

mv /tmp/notification.status.new /tmp/notification.status.old

# Any msgs can be appended on
# task -l|grep -vi "^--\|^$\|tasks\$\|task\$" > ~/.notification.status

# Futu live news 7X24
curl 'https://news.futunn.com/main/live-list?page=0&page_size=10' \
  ...
  -s \
  --compressed|jq ".data.list"|jq '.[]'|jq '.title, .content' \
  > /tmp/notification.status.new

cat /tmp/notification.status.new /tmp/notification.status.old|sort|uniq -u > /tmp/notification.status
ct=$(cat /tmp/notification.status|wc -l)
if [ "$ct" == "0" ]; then
  :> $FILEPATH
  task -l|grep -vi "^--\|^$\|tasks\$\|task\$" > /tmp/notification.status
fi
